            ...
            {!isNoHeadCategory ? (
              <div>
                ...
                <select className={inputCls('headCategory')} value={form.headCategory} onChange={set('headCategory')}>
                  ...
                  {getHeadCategoriesForAppType(form.applicationType).map(hc => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:items-center flex-wrap gap-2">
            ...
            {hasFilters && (
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center space-x-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 transition-colors min-h-[38px]"
              >
        ...
        <div className="flex items-center text-xs gap-2 flex-wrap">
          ...
          {filterCategory !== 'All' && (
            <span className="rounded-full bg-cyan-50 dark:bg-cyan-500/20 px-2 py-0.5 text-cyan-700 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-500/20">
              Category: {getHeadCategoryDisplayName(filterCategory, filterType)}
            </span>
          )}
          {filterStatus !== 'All' && (
